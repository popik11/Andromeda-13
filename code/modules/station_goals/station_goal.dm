...
/datum/station_goal/proc/send_report()
	priority_announce("Приоритетная директива Нанотразен получена. Получены подробности проекта \"[name]\".", "Входящее приоритетное сообщение", SSstation.announcer.get_rand_report_sound())
	print_command_report(get_report(),"Директива Нанотразен [pick(GLOB.phonetic_alphabet)] \Roman[rand(1,50)]", announce=FALSE)
	on_report()
