...
/obj/machinery/satellite/meteor_shield/proc/handle_new_emagged_shield_threshold()
	switch(highest_emagged_threshold_reached)
		if(EMAGGED_METEOR_SHIELD_THRESHOLD_ONE)
			say("Внимание. Вероятность метеоритного удара входит в опасные диапазоны для более экзотических метеоров.")
		if(EMAGGED_METEOR_SHIELD_THRESHOLD_TWO)
			say("Предупреждение. Опасность попадания темной материи в существующие диапазоны. О дальнейшем вмешательстве будет сообщено.")
		if(EMAGGED_METEOR_SHIELD_THRESHOLD_THREE)
			say("Внимание. Поступило сообщение о дальнейших взломах.")
			priority_announce("Внимание. В результате вмешательства в работу метеоритных спутников станция подвергается риску экзотических, смертоносных столкновений с метеоритами. Пожалуйста, вмешайтесь, проверьте свои GPS-устройства на наличие странных сигналов и демонтируйте испорченные метеоритные щиты.", "Предупреждение о странном метеоритном сигнале")
		if(EMAGGED_METEOR_SHIELD_THRESHOLD_FOUR)
			say("Внимание. Внимание. Тёмный Матт-Эор держит курс на станцию.")
			force_event_async(/datum/round_event_control/dark_matteor, "массив поддельных метеоритных спутников")
