...
/datum/sm_delam/proc/effect_explosion(obj/machinery/power/supermatter_crystal/sm)
	...
	explosion(origin = sm_turf,
		devastation_range = calculate_explosion(sm) * 5.0, ///диапазон разрушений	//max 17.5
		heavy_impact_range = calculate_explosion(sm) + 12, ///диапазон сильных воздействий	//max 37
		light_impact_range = calculate_explosion(sm) + 32, ///диапазон легких воздействий	//max 39
		flash_range = calculate_explosion(sm) + 12, ///диапазон шариков аномалии	//max 41
		adminlog = TRUE,
		ignorecap = TRUE
	)
	return TRUE
	...
	*/
	...
	if(SSshuttle.emergency.mode != SHUTTLE_ESCAPE)
		priority_announce(
			...
			sender_override = "Аварийное оповещение о состоянии шаттла шаттла",
			color_override = "grey",
		)
	else
	// except if you are on it already, then you are safe c:
		minor_announce("ОШИБКА: Обнаружено повреждение в навигационных протоколах. Соединение с транспондером #XCC-P5831-ES13 потеряно. \
				Расшифрован протокол резервного маршрута выхода. Калибровка маршрута...",
...
/datum/sm_delam/proc/effect_evac_rift_end()
	...
	if(SSshuttle.emergency.mode == SHUTTLE_ESCAPE)
		...
		var/shuttle_msg = "Навигационный протокол установлен на [SSshuttle.emergency.is_hijacked() ? "\[ОШИБКА\]" : "резервный маршрут"]. \
			Переориентация корабля блюспейса на вектор выхода. Расчетное время прибытия 15 секунд."
