...
/obj/structure/carp_rift/proc/update_check()
	...
	if(time_charged >= max_charge)
		...
		priority_announce("Пространственный объект достиг пикового заряда энергии в [initial(A.name)], пожалуйста, будьте готовы.", "[command_name()]", has_important_message = TRUE)
	...
	if(charge_state < CHARGE_FINALWARNING && time_charged >= (max_charge * 0.5))
		...
		var/area/A = get_area(src)
		priority_announce("Разлом вызывает неестественно большой поток энергии в [initial(A.name)]. Остановите его любой ценой!", "[command_name()]", ANNOUNCER_SPANOMALIES)
