...
/datum/surgery_step/add_prosthetic/success(mob/user, mob/living/carbon/target, target_zone, obj/item/tool, datum/surgery/surgery, default_display_results = FALSE)
	...
	if(isbodypart(tool) && user.temporarilyRemoveItemFromInventory(tool))
		...
		display_pain(target, "Вы наполняетесь позитивными ощущениями, потому что вы снова чувствуете вашу [target.parse_zone_with_bodypart(target_zone)]!", TRUE)
		return
	else
		...
		user.visible_message(span_notice("[user] заканчивает прикреплять [tool]!"), span_notice("Вы прикрепили [tool]."))
		display_results(
			user,
			target,
			span_notice("Вы прикрепляете [tool]."),
			span_notice("[user] прикрепляет [tool]!"),
			span_notice("[user] завершает операцию по прикреплению!"),
		)
		display_pain(target, "Вы испытываете странные ощущения от присоединения вашей [target.parse_zone_with_bodypart(target_zone)].", TRUE)
