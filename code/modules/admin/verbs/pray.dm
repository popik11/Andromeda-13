/mob/verb/pray(msg as text)
	...
	if(GLOB.say_disabled) //This is here to try to identify lag problems
		to_chat(usr, span_danger("В настоящее время молитва отключена администратором."), confidential = TRUE)
		return
	...
	var/prayer_type = "МОЛИТВА"
	var/deity
	if(usr.job == JOB_CHAPLAIN)
		cross.icon_state = "kingyellow"
		font_color = "blue"
		prayer_type = "МОЛИТВА СВЯЩЕННИКА"
		if(GLOB.deity)
			deity = GLOB.deity
	else if(IS_CULTIST(usr))
		cross.icon_state = "tome"
		font_color = "red"
		prayer_type = "МОЛИТВА КУЛЬТИСТА"
		deity = "Нар'Си"
...
/proc/message_centcom(text, mob/sender)
	...
	msg = span_adminnotice("<b><font color=orange>ЦЕНТКОМ:</font>[ADMIN_FULLMONTY(sender)] [ADMIN_CENTCOM_REPLY(sender)]:</b> [msg]")
...
/proc/message_syndicate(text, mob/sender)
	...
	msg = span_adminnotice("<b><font color=crimson>СИНДИКАТ:</font>[ADMIN_FULLMONTY(sender)] [ADMIN_SYNDICATE_REPLY(sender)]:</b> [msg]")
	for(var/client/staff as anything in GLOB.admins)
		if(staff?.prefs.read_preference(/datum/preference/toggle/comms_notification))
			SEND_SOUND(staff, sound('sound/misc/server-ready.ogg'))
	to_chat(GLOB.admins, msg, type = MESSAGE_TYPE_PRAYER, confidential = TRUE)
...
/proc/nuke_request(text, mob/sender)
	...
	msg = span_adminnotice("<b><font color=orange>ЗАПРОС ЯДЕРНОГО КОДА:</font>[ADMIN_FULLMONTY(sender)] [ADMIN_CENTCOM_REPLY(sender)] [ADMIN_SET_SD_CODE]:</b> [msg]")
	for(var/client/staff as anything in GLOB.admins)
		SEND_SOUND(staff, sound('sound/misc/server-ready.ogg'))
	to_chat(GLOB.admins, msg, type = MESSAGE_TYPE_PRAYER, confidential = TRUE)
	for(var/obj/machinery/computer/communications/console in GLOB.shuttle_caller_list)
