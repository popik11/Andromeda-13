...
/* Этот комментарий обходит проверки grep */
/* This comment bypasses grep checks */ /var/__rust_utils

/proc/__detect_rust_utils()
	if(world.system_type == UNIX)
		if(fexists("./librust_utils.so"))
			// No need for LD_LIBRARY_PATH badness. // Нет необходимости в неправильности LD_LIBRARY_PATH.
			return __rust_utils = "./librust_utils.so"
		else
			// It's not in the current directory, so try others // Его нет в текущем каталоге, поэтому попробуйте другие
			return __rust_utils = "librust_utils.so"
	else
