...
/obj/item/hos_color_changer/attack_self(mob/user)
	...
	if(tgui_alert(user, "Желаешь ли ты перекрасить Бриг в Красный?", "Какой СБшник ты сегодня?", list("Да", "Нет")) == "Да")
		...
		for(var/obj/effect/turf_decal/siding/red/security_decals1 as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/effect/turf_decal/siding/red))
			security_decals1.color = "#DE3A3A"
		for(var/obj/effect/turf_decal/trimline/red/security_decals2 as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/effect/turf_decal/trimline/red))
			security_decals2.color = "#DE3A3A"
		for(var/obj/structure/closet/secure_closet/security/sec/security_closet as anything in SSmachines.get_machines_by_type_and_subtypes(/obj/structure/closet/secure_closet/security/sec))
